...
function initVipPage() {
  ...
  function applyStatus(status) {
    ...
    if (status.status === "pending") {
      badge.textContent = "Заявка отправлена ✓ — ожидайте звонка";
      // Кнопку НЕ блокируем — клиент может заказать снова
      if (submitBtn) { submitBtn.textContent = "Отправить ещё одну заявку"; submitBtn.disabled = false; }
  ...
  function setupFirebase() {
    ...
    // Кнопка заявки — можно нажимать неограниченное число раз
    const submitBtn = document.querySelector("[data-vip-submit]");
    if (submitBtn) {
      submitBtn.addEventListener("click", () => {
        ...
        // Считаем количество заявок от этого клиента
        const orderCount = (status.orderCount || 0) + 1;
        ...
        fd.append("🏆 Тип клиента",   "VIP-клиент");
        ...
        fd.append("Номер заявки",     `#${orderCount}`);
        fd.append("Дата",             new Date().toLocaleString("ru-RU"));

        fetch("https://formspree.io/f/xojoparp", {
          method: "POST", headers: { Accept: "application/json" }, body: fd,
        })
          .then((r) => {
            if (r.ok) {
              submitBtn.textContent = `✓ Заявка #${orderCount} отправлена`;
            } else {
              submitBtn.textContent = "Ошибка — попробуйте ещё раз";
            }
          })
          .catch(() => {
            submitBtn.textContent = "Нет соединения — попробуйте ещё раз";
          })
          .finally(() => {
            // Через 3 секунды кнопка снова доступна для повторного заказа
            setTimeout(() => {
              submitBtn.textContent = "Отправить ещё одну заявку";
