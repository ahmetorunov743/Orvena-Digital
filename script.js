...
const VIP_USER_CACHE  = "orvena-vip-user"; // быстрый кэш для мгновенного входа
...
function initVipPage() {
  ...
  function applyStatus(status) {
    ...
    // Убираем старую кнопку сброса, если была
    const oldReset = dashboard.querySelector("[data-vip-reset]");
    ...
    if (status.status === "pending") {
      ...
      // Добавляем кнопку "Изменить и отправить снова"
      const resetBtn = document.createElement("button");
      resetBtn.type = "button";
      resetBtn.className = "vip-reset-btn";
      resetBtn.dataset.vipReset = "";
      resetBtn.textContent = "Изменить параметры и отправить снова";
      submitBtn.insertAdjacentElement("afterend", resetBtn);
      resetBtn.addEventListener("click", () => {
        const s = getVipStatus();
        s.status = "none";
  ...
  // ---------- Кэш пользователя для мгновенного входа ----------
  function getCachedUser() {
    try { return JSON.parse(localStorage.getItem(VIP_USER_CACHE) || "null"); }
    catch { return null; }
  }
  function setCachedUser(user) {
    if (user) {
      localStorage.setItem(VIP_USER_CACHE, JSON.stringify({
        displayName: user.displayName || null,
        email: user.email || null,
      }));
    } else {
      localStorage.removeItem(VIP_USER_CACHE);
    }
  }
  ...
  function setupFirebase() {
    ...
    // Если пользователь уже заходил — показываем дашборд мгновенно из кэша,
    // не ждём ответа Firebase (обычно это занимало 1-2 секунды и более).
    // Firebase всё равно проверит токен в фоне и уточнит данные.
    ...
    window.__fbOnAuth((user) => {
      setCachedUser(user); // обновляем кэш
      if (user) {
        showDash(user);    // обновляем данные (имя/email могли измениться)
      } else {
        // Firebase говорит: пользователь вышел / токен истёк
        showGate();
      }
    });
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", () => {
        ...
        // ⚠️ ВАЖНО: замените на ОТДЕЛЬНЫЙ Formspree-эндпоинт для Orvena Digital
        // (не тот же что WebCraft, иначе заявки будут смешиваться в одном ящике)
        // Как получить отдельный: зайдите на formspree.io, создайте новую форму
        // под email, на который хотите получать VIP-заявки, и вставьте сюда её ID.
        const VIP_FORMSPREE = "https://formspree.io/f/ЗАМЕНИТЕ_НА_СВОЙ_ID";
        fetch(VIP_FORMSPREE, {
    ...
    if (signoutBtn) {
      signoutBtn.addEventListener("click", () => {
        window.__fbSignOut().then(() => {
          clearVipStatus();
          setCachedUser(null); // очищаем кэш — при следующем визите не попадёт сразу в кабинет
